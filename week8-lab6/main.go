...
func getAllBooks(c *gin.Context) {
	...
	hasYear := false
	if yearInput != "" {
		yy, convErr := strconv.Atoi(yearInput)
		if convErr != nil {
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			&b.Year, &b.Price, &b.CreatedAt, &b.UpdatedAt,
		); err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
			return
		}
	...
	// ถ้ามีตัวกรองปี และไม่พบข้อมูล → 404
	if hasYear && len(books) == 0 {
		c.JSON(http.StatusNotFound, gin.H{"error": fmt.Sprintf("no books found for year=%d", y)})
		return
	}
