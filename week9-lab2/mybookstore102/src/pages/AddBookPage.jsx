...
const AddBookPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`${apiBase}/api/v1/books`, {
        method: 'POST',
      ...
      setFormData({ title: '', author: '', isbn: '', year: '', price: '' });
      setTimeout(() => setSuccessMessage(''), 5000);

      // navigate('/store-manager/books'); // ถ้าจะเด้งกลับทันทีให้เอา comment ออก
    } catch (error) {
      setErrors(prev => ({ ...prev, submit: 'เกิดข้อผิดพลาดในการเพิ่มหนังสือ: ' + error.message }));
    } finally {
